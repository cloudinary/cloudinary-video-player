const url = 'http://localhost:3000/recommendations/recommendations.html';
...
describe('Recommendations tests', () => {
  ...
  it('Test recommendations visibility', async () => {
    ...
    const duration = Math.floor(await page.evaluate(() => player.duration()));
    await page.waitFor(1000);
  ...
  it('Test recommendations click', async () => {
    ...
    await page.waitFor(1000);
    const recSourceUrl = await page.evaluate(() => player.currentSourceUrl());

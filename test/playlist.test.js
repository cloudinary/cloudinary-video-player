describe('Playlist tests', () => {
  ...
  it('Test video change', async () => {
    await page.waitForSelector('#example-player_html5_api', { visible: true, timeout: 35000 });
    let curruntVideoUrl = await page.$eval('#example-player_html5_api', vid => vid.src);
    await page.click('#fluid-demo-container > div:nth-child(2) > div > div.cld-plw-col-list > div > a:nth-child(2)');
    let nextVideoUrl = await page.$eval('#example-player_html5_api', vid => vid.src);

...
for (const link of ESM_LINKS) {
    vpTest.only(`Test console errors on link ${link.name}`, async ({ page, consoleErrors, vpExamples }) => {
        ...
        if (process.env.PREVIEW_URL && !PREVIEW_URL_LOADED) {
            await expect(async () => {
                ...
                console.log('wait for link to be visible: ', await linkLocator.isVisible());
                await expect(linkLocator).toBeVisible({ timeout: 3000 });
